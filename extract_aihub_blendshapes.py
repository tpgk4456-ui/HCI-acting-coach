...
# 중립 이미지 폴더 경로
neutral_image_dir = base_dir / "raw" / "EMOIMG_중립_VALID"

# 분노 이미지 폴더 경로
anger_image_dir = base_dir / "raw" / "EMOIMG_분노_VALID"
...
output_csv = "aihub_neutral_anger_blendshapes.csv"

# 감정별 분포 요약 저장 파일
neutral_distribution_csv = "neutral_distribution.csv"
anger_distribution_csv = "anger_distribution.csv"
...
# 경로 확인 로그
print("중립 폴더 존재:", neutral_image_dir.exists())
print("분노 폴더 존재:", anger_image_dir.exists())

# 중립 이미지 처리
neutral_results = process_emotion_images(
    neutral_image_dir,
    "neutral",
...
# 분노 이미지 처리
anger_results = process_emotion_images(
    anger_image_dir,
    "anger",
    sample_size_per_emotion
)
...
# 두 감정 결과 합치기
all_results = neutral_results + anger_results
...
# 저장 로그 출력
print(f"전체 blendshape CSV 저장 완료: {output_csv}")
print(f"총 저장 개수: {len(df)}")

# 중립 분포 저장
save_distribution(df, "neutral", neutral_distribution_csv)

# 분노 분포 저장
save_distribution(df, "anger", anger_distribution_csv)
